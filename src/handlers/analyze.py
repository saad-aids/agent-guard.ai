"""
Lambda handler for POST /v1/analyze-log.

Implements the 9-step analysis flow:
  1. Authenticate via X-API-Key header
  2. Validate request body (developer_id + payload)
  3. Estimate tokens
  4. Calculate cost
  5. Classify risk
  6. Fetch developer's budget rule
...
def lambda_handler(event: dict, context) -> dict:  # noqa: ANN001
    """Entry point for POST /v1/analyze-log.
    ...
    # ------------------------------------------------------------------
    # Step 9: Return 200 with analysis result
    # ------------------------------------------------------------------
    response_body: dict = {
        "developer_id": developer_id,
        "tokens_estimated": tokens_estimated,
